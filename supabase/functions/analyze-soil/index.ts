...
serve(async (req) => {
  ...
  try {
    ...
    if (!CLAUDE_API_KEY || CLAUDE_API_KEY.includes("your-key") || CLAUDE_API_KEY === "mock") {
      console.log("Using Mock/Offline Soil Classification Fallback");
      const mockResult = {
        primarySoilType: "CLAY",
        secondaryDescriptors: ["Silty"],
        plasticity: "medium",
        colour: "brown",
        colourBecoming: "",
        minorComponents: ["trace sand", "with rootlets"],
        confidence: "medium",
        notes: "Mock offline classification: High consistency brown silty clay detected."
      };
